...
export class KeysApp extends HTMLElement {
  ...
  renderDetails(){const panel=this.querySelector('#details');panel.innerHTML=details(this.state,{...this.musicalContext(),selected:this.selected,quiz:this.quiz,focusChord:this.focusChord});panel.hidden=!panel.innerHTML.trim();this.querySelector('.learning-panel').dataset.mode=this.state.mode;this.querySelector('.learning-panel').classList.toggle('has-details',!panel.hidden);}
  onInput(e){
